...
_SUPPORTED_UPLOAD_TYPES = {"video/mp4", "video/x-matroska", "video/quicktime"}
...
@post(
    ...
    description="Generate a presigned PUT URL for direct video upload to object storage.",
)
async def presign(data: PresignInput) -> PresignResult:
    if data.content_type not in _SUPPORTED_UPLOAD_TYPES:
